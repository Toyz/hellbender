...
fn frame(stem: &str, out: &str) -> Option<usize> {
    let level = hb_render::Level::from_disc(stem)?;
...
fn nothing_falls_on_a_clear_level() {
    let Some(level) = hb_render::Level::from_disc("jurasic") else { return };
...
fn a_flash_lights_iowahs_sky_and_its_bolt() {
    let Some(level) = hb_render::Level::from_disc("iowah") else { return };
